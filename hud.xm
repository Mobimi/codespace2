...
- (instancetype)initWithFrame:(CGRect)frame title:(NSString *)title {
    ...
    self.bgLayer.strokeColor = [UIColor colorWithWhite:0.5 alpha:0.4].CGColor;
...
// ═══════════════════════════════════════════════
//  COMPONENT 2: ROOT VC (FORWARD ORIENTATION)
// ═══════════════════════════════════════════════
...
- (UIInterfaceOrientationMask)supportedInterfaceOrientations {
    for (UIWindowScene *sc in [UIApplication sharedApplication].connectedScenes) {
        ...
        for (UIWindow *w in [(UIWindowScene *)sc windows]) {
            if ([NSStringFromClass([w class]) containsString:@"HUD"]) continue;
            if ([NSStringFromClass([w class]) containsString:@"GOMenu"]) continue;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    self.rootViewController = [[HUDRootVC alloc] init];

    // Panel
    self.hudPanel = [[UIView alloc] initWithFrame:CGRectMake(20, 50, 145, 55)];
    self.hudPanel.backgroundColor    = [UIColor colorWithWhite:0.0 alpha:0.45];
    self.hudPanel.layer.cornerRadius = 12;
    self.hudPanel.layer.borderWidth  = 0.6;
    self.hudPanel.layer.borderColor  = [UIColor colorWithWhite:1.0 alpha:0.15].CGColor;
    ...
    // Ẩn ban đầu, chờ switch bật
    [self updateVisibility];

    // Lắng nghe thay đổi switch từ menu
...
- (void)updateVisibility {
    BOOL on = [GO_PREFS boolForKey:@"GO_PerfHUD"];
...
- (void)handlePan:(UIPanGestureRecognizer *)r {
    ...
    self.hudPanel.center = CGPointMake(self.hudPanel.center.x + d.x,
                                       self.hudPanel.center.y + d.y);
...
- (void)tick:(CADisplayLink *)link {
    ...
    self.fpsValue.textColor = fps >= 55 ? [UIColor greenColor] :
                              fps >= 30 ? [UIColor yellowColor] :
                                          [UIColor redColor];
    ...
    double ramMB = 0;
    if (task_info(mach_task_self(), MACH_TASK_BASIC_INFO, (task_info_t)&info, &sz) == KERN_SUCCESS)
        ramMB = info.resident_size / 1024.0 / 1024.0;
    [self.ramView updateWithProgress:ramMB / 6144.0
    ...
    if (task_threads(mach_task_self(), &threads, &threadCount) == KERN_SUCCESS) {
        for (mach_msg_type_number_t i = 0; i < threadCount; i++) {
            ...
            if (thread_info(threads[i], THREAD_BASIC_INFO, (thread_info_t)thinfo, &infoCount) == KERN_SUCCESS) {
                thread_basic_info_t basic = (thread_basic_info_t)thinfo;
                if (!(basic->flags & TH_FLAGS_IDLE))
                    totalCPU += basic->cpu_usage / (float)TH_USAGE_SCALE * 100.0;
            }
        }
    ...
    float cpu = MIN(totalCPU / cores, 100.0);
    [self.cpuView updateWithProgress:cpu / 100.0
                           valueText:[NSString stringWithFormat:@"%.0f%%", cpu]];
}
