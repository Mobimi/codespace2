...
return (saved > 0.1f) ? (CGFloat)saved : 3.0f;
}
...
- (void)populateMenu {
  ...
  @{@“name”: @“🎨  Render Optimize”,     @“key”: @“GO_RenderOpt”},
  ];
  ...
  // ── Drawable Count Row ──
  UIView *drawableRow = [[UIView alloc] initWithFrame:CGRectMake(0, yOffset, 260, 44)];
  drawableRow.backgroundColor    = CLR_ROW;
  drawableRow.layer.cornerRadius = 8;
  
  UILabel *drawableLbl = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 130, 44)];
  drawableLbl.text      = @“🎮  Drawable Count”;
  drawableLbl.textColor = CLR_TEXT;
  drawableLbl.font      = [UIFont systemFontOfSize:13];
  [drawableRow addSubview:drawableLbl];
  
  UISegmentedControl *seg = [[UISegmentedControl alloc] initWithItems:@[@“2”, @“3”]];
  seg.frame = CGRectMake(148, 9, 102, 28);
  seg.tintColor = CLR_ACCENT;
  NSInteger savedDrawable = [prefs integerForKey:@“GO_DrawableCount”];
  seg.selectedSegmentIndex = (savedDrawable == 3) ? 1 : 0; // mặc định 2
  [seg addTarget:self
  action:@selector(drawableChanged:)
  forControlEvents:UIControlEventValueChanged];
  [drawableRow addSubview:seg];
  
  [self.scrollView addSubview:drawableRow];
  yOffset += 52;
...
- (void)drawableChanged:(UISegmentedControl *)seg {
  NSInteger val = (seg.selectedSegmentIndex == 1) ? 3 : 2;
  [GO_PREFS setInteger:val forKey:@“GO_DrawableCount”];
  }
...
**attribute**((constructor)) static void go_init() {
@autoreleasepool {
